...
/* SOURCE : http://codeforces.com/blog/entry/21853 */
struct pair_hash {
    template <class T1, class T2>
    std::size_t operator () (const std::pair<T1,T2> &p) const {
        return hash<long>()(((long)p.first)^(((long)p.second)<<32));
    }
};

typedef unordered_map<pair<ulint,ulint>,pair<int,ulint*>,pair_hash> maperino;
maperino::size_type n = 10000;
//maperino m(n, pair_hash(), pair_equal())
...
class Graph{
	...
	public:
		...
		inline maperino* getEdges(){ return _edges;}

		Graph(vector<Node*> *nodes, maperino* edges){
			_nodes=nodes;
			//cout << _nodes;
			//_m=m;
			_edges=edges;
...
Graph* parse() {
	...
	maperino* edges = new maperino(n, pair_hash()); //ESTÁ PARA PONTEIRO NAO PARA INT
